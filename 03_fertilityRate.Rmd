...
static_lineChart <- function(
  data, 
  xaxisPadding = 14, 
  label.font = "Open Sans Condensed Light",
  hbox = 2.1
  ) {
  ...
  gp <- ggplot(data = dd) +  
   ...
   # scale_y_continuous(limits = c(0, max(data$value))) + 
    ylab("") + 
    coord_fixed(ratio = 7.7)

    # if(!is.null(hbox)) {
    #   gp <- gp +
    #     geom_rect_interactive(
    #       data = data.frame(xmin = minYear, xmax = maxYear,
    #       ymin = 0, ymax = hbox, tooltip = "asfsadf", id = "id"),
    #       mapping = aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax,
    #         tooltip = tooltip, data_id = id), fill = "grey", alpha = 0.3)
    # }
    ...
    scale_alpha_manual(values = c(0.28, 0.7), guide=FALSE) +
...
interactive_chart <- function(gpath, fontname = 'Open Sans Condensed Light') {
  ggiraph(
    code = {print(gpath)},
    hover_css = "stroke-opacity:0.99;stroke-width:2px;stroke:black;",
    tooltip_opacity = 0.7,
    pointsize = 15,
    width = "100%",
    #height = "500px",
